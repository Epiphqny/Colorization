...
from data_loader_colornet import ValImageFolder
import numpy as np
...
from training_layers_colornet import decode
...
def load_image(image_path,transform=None):
    ...
    image_small=np.expand_dims(rgb2lab(image)[:,:,0],axis=-1)
...
def main():
    ...
    color_model = nn.DataParallel(ColorNet()).cuda().eval()
    color_model.load_state_dict(torch.load('../model/models_colornet/model-1-400.ckpt'))
